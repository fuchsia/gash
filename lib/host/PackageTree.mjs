...
import Scriptlet,{PACKAGE_KEY_JS_HELL,PACKAGE_KEY_JS_HELL_LEGACY,PACKAGE_KEY} from "./Scriptlet.mjs";
...
import CommandLineScript,{sh} from "./CommandLineScript.mjs";
const DEBUG = false;
...
getPackageJsonKey( packageJson ) {
    let found = false,
        keyName = '';
    for ( const k of [ PACKAGE_KEY, PACKAGE_KEY_JS_HELL, PACKAGE_KEY_JS_HELL_LEGACY ] ) {
        if ( Object.hasOwn( packageJson, k ) ) {
            if ( found ) {
                throw new TypeError( `package cannot have mix \`${PACKAGE_KEY}\`, \`${PACKAGE_KEY_JS_HELL}\` and \`${PACKAGE_KEY_JS_HELL_LEGACY}\` keys; pick one.` );
            }
            found = true; 
            keyName = k;
        }
    }
    if ( keyName !== PACKAGE_KEY && keyName ) {
        // This is unhelpful as we can't pinpoint the file; we really need that.
        // Q: Should we use process.emitWarning()?
        false && console.warn( "DEPRECATED: use of `%s` as package key", keyName );
    }  
    return keyName;
} 
...
getRunnableScript( scriptText ) {
    ...
    const LEGACY_PREFIX = "js-hell ";
    const PREFIX = NEW_NAME + " ";
    const t = `${scriptText}`.trimStart(); 
    if ( t.startsWith( LEGACY_PREFIX ) || t.startsWith( PREFIX ) ) {
...
// 2025_2_15: @brief Used for blank scriptlets - solely so we can check whether
// it's using `sh` correctly. Delete once `sh` becomes a normal global. 
class 
BlankScriptletNode extends ScriptletNode {
    
    constructor( scriptlet, options ) {
        super( scriptlet, options );
    }
    toScriptlet() {
        if ( !this.value.idl.isValidGashUsage() ) {
            throw new Error( "`gash()` cannot be used in a `with()` statement" ); 
        }
        return this.value; 
    }
};
        ...
        if ( type === URLTYPE_IGNORE ) {
            ...
            nodes.push( new ScriptletNode( scriptlet, { type, source } ) );
             ...
             if ( IDL_isNested( idl ) ) {
                ...
                let module;
                // 2025_2_1: If we always supply the blank module, then `Scriptlet` doesn't
                // have to understand the `blankjs` and it becomes an internal feature of us.
                const isBlankModule = isBlankModuleUrl( url ); 
                if ( isBlankModule ) {
                    module = Object.create( null );
                    // 2025_2_1: `gash()` is global that is only available when you don't import
                    // other modules. Making it an export of the "blank" module means you don't have 
                    // to write `with() gash()` as you would for a normal global. (Maybe this could 
                    // be generalised?) 
                    module.gash = sh;
                }
                const scriptlet = new Scriptlet( idl, { module, moduleUrl: url, version } );
                // Ugly. But this is a temp hack till `sh()` is safe for nested use. (`sh( 'x', sh( 'y' ) )` etc...) 
                nodes.push( new (isBlankModule ? BlankScriptletNode : ScriptletNode)( scriptlet, { type, source } ) );
             } 
        } else {
...
PackageTree
{
    ...
    async init() {
        ...
        {
            const scriptlet = new Scriptlet( JsHell.js_hell, {module:JsHell,name:NEW_NAME});
            scriptlet.idl.addTail(); // This does lots of sanity checking.
            this.addScriptlet( NODETYPE_MODULE, scriptlet, SOURCE_BUILTIN );
        }
    }
