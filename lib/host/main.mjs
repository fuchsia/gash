...
import VersionError from "./VersionError.mjs";
...
installGlobals() {
    ...
    } else if ( globalThis.alert !== alert ) {
        console.warn( "global `alert`: already exists, and won't be gash's version." );
    }
    if ( typeof globalThis.prompt === 'undefined' ) {
        globalThis.prompt = prompt;
    } else if ( globalThis.prompt !== prompt ) {
        console.warn( "global `prompt`: already exists, and won't be gash's version." );
    }
    if ( typeof globalThis.confirm === 'undefined' ) {
        globalThis.confirm = confirm;
    } else if ( globalThis.confirm !== confirm ) {
        console.warn( "global `confirm`: already exists, and won't be gash's version." );
    }
}
...
parseInvocation( argIterator, { stacktrace = false, inspect = false, reparse = false, head = true, pipeTail = false, jsHellAllowed = false, versionAllowed = false } = {}  ) {
    ...
    function* iteratorCopy( ) {
        for ( let first = true;; first = false) {
            ...
            if ( !USE_JS_HELL_SCRIPTLET && jsHellAllowed && arg.type === ARG_POSITIONAL_VALUE && ( arg.value === "js-hell" || arg.value === NEW_NAME )
...
Shell {
    ...
    //
    // Replace with a `createJob()` paradign where execution begins on call to a "fetch result" method.
    async execJob( _argv, { capture, startupDir = this.pwd() } ) {
        ...
        try {
            ...
            RUN: for( let {node,pipeHead,pipeTail} = walker.next(); node; {node,pipeHead,pipeTail} = walker.next( lastResult ) ) {
                ...
                try {
                    ...
                    if ( success ) {
                        ...
                        // Should the error be in the job?
                        node.error( value );
            ...
            /*getCurrentShellJob().error( err, stacktrace );*/
            ...
            console.error( NEW_NAME + ": %s", stacktrace && err.type !== 'parse' && err.name !== VersionError.name ? err.stack : err.message ); 
...
main( {argv = process.argv,capture,startupDir,...shellOptions} = {} ) {
    ...
    if ( typeof curJob === 'undefined' ) {
        ...
        try {
            ...
            shell.finalise();
            // Q: Should we uninstall the console as well?
            // A: If anther invocation() of main has happened, it will be depending on it. (We could 
            // ref count installations?)
        }
    } else {
