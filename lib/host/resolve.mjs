...
import Scriptlet,{IdlText_fromModule,MODULE_KEY} from "./Scriptlet.mjs";
...
Scriptlet_fromEsmModule( url, shim )
{
    ...
    /// - 2025_1_31: Removed the old number API version support.
    ///
    /// - FIXME: this should be an error if `shim` is defined, shouldn't it?
    /// Or should it always override?
    ///
    const idlText = IdlText_fromModule( module );
    if ( typeof idlText === 'string' ) 
        return Scriptlet.from( idlText, url, module );
    // FIXME: what counts as a shim?  
    if ( shim ) 
        return Scriptlet.from( shim, url, module );
    throw new TypeError( `Incompatible module (missing or invalid "${MODULE_KEY}" export in module ${JSON.stringify( url )})` );
}
async function
