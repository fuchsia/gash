...
import {fullstack,fullStackException,fullStackSuccess} from "./fullstack.spec.mjs";
...
describe( "the host", () => {
    ...
    it( "should handle an builtin command that's encoded as a single arg", async () => {
        // 2025_2_13: Can we not replace this with `fullStackSuccess`? 
        let result = '';
        const stdout = {
    ...
    it( "should reject an unsupported CLI=x version", async () => {
        expect( await fullStackException( "CLI=1c echo hello world" ) ).toEqual( NEW_NAME + ": Unsupported CLI version (1c)" );
    } );
...
*/
    describe( "should handle a pipe", () => {
        ...
        it( "failing when the file topic is explicitly used to replace a file", async () => {
            expect( await fullStackException( "echo 'P45' | ./test-data/say-string.mjs -" ) ).toEqual( NEW_NAME + `: cannot pipe to "./test-data/say-string.mjs"` );
        } );
        ...
        it( "failing when used twice [PIPE-FILETOPIC-ONCE-ONLY]", async () => {
            expect( await fullStackException( "echo 'P45' | ./test-data/cat2.mjs - -" ) ).toMatch( /^[^:]+: the file topic \(`-`\) can only be used ONCE in the command line / );
        } );
        it( "failing if --output is also used [PIPE-TEELESS]", async () => {
            expect( await fullStackException( "echo 'P45' --output=tmp.txt | cat -" ) ).toMatch( /^[^:]+: cannot use `--output` inside a pipe/ );
        } );
    } );
    ...
    it( "user error should be reported [EXIT-USER-EX]", async () => {
        ...
        expect( EXIT_SCRIPTLET_EXCEPTION ).toEqual( 2 ); // This is hard coded. in the docs.
        expect( result ).toEqual( '' );
