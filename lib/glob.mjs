...
import safeStatSync from "./utils/safeStatSync.mjs";
...
glob( singleOrMultipleFilespecs, { recurse, cwd=".", glob: defaultGlob="*", exclude = "", return: { dirs: returnDirs = false } = {}, minDate } = {} )
    {
        cwd = `${cwd}`;
            ...
            for ( let i = 0; i < dirs.length; ++i ) {
                ...
                for ( const f of readdirSync( dir, {withFileTypes:true} ) ) {
                    ...
                    if ( excludeRegex && excludeRegex.test( f.name ) )
                        continue;
                    // 2025_4_10: NB is now a `parentPath` element (as of node 18.20,20.12,21.4) so we may not need to track
                    // dir ourselves.
                    ...
                    if ( f.isDirectory() ) {
                        ...
                        if ( globRegex.test( f.name ) ) {
                            // We should probably allow numbers here, anyway.
                            if ( typeof minDate !== 'undefined' ) {
                                if ( !( safeStatSync( filename )?.mtimeMs >= minDate ) ) {
                                    continue;
                                }
                            }
