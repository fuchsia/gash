...
/// @brief Is the node an `[await] x()` Used to validate the gash node is as we
/// expect.
export function
isCall( node, {await:allowAwait = false} = {} ) {
    if ( node.type === TYPE_UNARY && allowAwait ) {
        if ( node.op !== UNARY_SYNC )
            return false;
        node = node.object;
    }
    return node.type === TYPE_CALL;
}
