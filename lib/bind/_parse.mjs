...
// 2025_2_14: This has been made misleading by the introduction of templates,
// as a template can _potentially_ be a non-literal.
function 
...
optionallyReadDefaultValue( instr, context )
    {
        if ( !instr.match( RE_OPERATOR_ASSIGNMENT, true ) )
            return null;
        const pos = instr.pos,
              // 2025_2_14: This simple literal could be a template; we need to check.
              result = optionallyReadSimpleLiteral( instr, context );
        if ( !result || !Ast_isLiteral( result ) )
...
readRightExpr( instr, node, context ) {
    if ( !instr.match( "??", true ) ) 
        return node;
    if ( Ast_isLiteral( node ) )
        throw instr.error( "Can't use a literal on the LHS of ??" );
    // 2022_9_26:
    // allow: `x ?? y`, `x[] ?? y`
...
readPipeExpr( instr, context, pipeIndex = 0 )
    {
        // NB `readRightExpr` is a misnomer; it returns the LeftExpr if there's no RightExpr;
        // and otherwise merges it into a binary. 
        ...
        if ( instr.match( RE_TERNARY, true ) ) {
            ...
            if ( Ast_isLiteral( conditionOrPipeHead ) )
                throw instr.error( "Can't use a literal on the LHS of a ternary" );
...
_parse( instr, parseAs = PARSE_AS_EXPR )
    {
        ...
        return { 
            ...
            nonWith: parseAsStatement && !isWithStatement //< bool:  true if we use the old syntax; needed for `sh()` cheecking. Delete once that removed. 2025_2_15:  
        };
    }
...
parseTemplateTail( instr, context, {raw, terminated} ) {
    ...
    if ( result.length === 1 ) {
        const node = result[0];
        if ( Ast_isLiteral( node ) && typeof node.value === 'string' )
            return node;
        // 2025_2_14: There appear to be no overt test cases for this; 
        // all of them rely on the above. But some of the SH-ARRAY-* 
        // tests indirectly cover it. 
        return createMethod( 'toString', node );
    }
    // Args.join()...
