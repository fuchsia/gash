import {sep as dirsep, join as Path_join, parse as Path_parse, resolve as Path_resolve} from "node:path";
...
import {refTextFile_name,refTextFile_basename,refTextFile_extlessName,refTextFile_fullPath,refTextFile_text,refTextFile_buffer,refTextFile_size, refTextFile_lastModified,
    refUrl_name,refUrl_text, refMissingFile_name, refMissingFile_type, refMissingFile_fullPath,
    refMissingFileExtless_name, 
    
    refJsonFile_name,refJsonFile_basename,refJsonFile_extlessName,refJsonFile_fullPath,refJsonFile_text,refJsonFile_value,refJsonFile_buffer, refJsonFile_size, refJsonFile_lastModified} from "./testdata.mjs";
...
for ( const fn of [ "FILE", "FILE_NAME", "FILENAME"] ) {
    describe( `${fn} should`, () => {
        ...
        it( "have a `dir` property that returns the absolute dir [FILE-DIR]", async () => {
            const {result} = await( new Idl( `IDL=1 cmd [--option=${fn}] ${fn} :: default( $1.dir, {option.dir} )` )
                ._exec( 
                    ( $1, { dir } ) => [ $1, dir ],
                    `cmd ${refTextFile_name} --option=${refJsonFile_name}`
                ) );
            expect( result ).toEqual( [ Path_parse( refTextFile_fullPath ).dir, Path_parse( refJsonFile_fullPath ).dir ] );
        } );
        it( "have an `ext` property that returns the file extension [FILE-EXT]", async () => {
            const {result} = await( new Idl( `IDL=1 cmd [--option=${fn}] ${fn} :: default( $1.ext, {option.ext} )` )
                ._exec( 
                    ( $1, { ext } ) => [ $1, ext ],
                    `cmd ${refTextFile_name} --option=${refJsonFile_name}`
                ) );
            expect( result ).toEqual( [ '.txt', '.json' ] );
        } );
        it( "have an `ext` property that returns nothing for `.x` files [FILE-DOTTED-NAME]", async () => {
            const {result} = await( new Idl( `IDL=1 cmd ${fn} :: default( $1.extlessName, $1.ext, $1.name )` )
                ._exec( 
                    ( $1, $2, $3 ) => [ $1, $2, $3 ],
                    `cmd ${Path_resolve( ".gitignore" )}`
                ) );
            expect( result ).toEqual( [ '.gitignore', '', '.gitignore' ] );
        } );
        it( "have an `extlessName` property that si the name [FILE-EXTLESSNAME]", async () => {
            const {result} = await( new Idl( `IDL=1 cmd [--option=${fn}] ${fn} :: default( $1.extlessName, {option.extlessName} )` )
                ._exec( 
                    ( $1, { extlessName } ) => [ $1, extlessName ],
                    `cmd ${refTextFile_name} --option=${refJsonFile_name}`
                ) );
            expect( result ).toEqual( [ refTextFile_extlessName, refJsonFile_extlessName ] );
        } );
